...
#define MAXSEQNUM		50000				/*maximum number of sequences even when fluctuating*/
#define MINSEQNUM		10000					/*minimum number of sequences even under population fluctuation*/
#define MAXGEN			500000				/*maximum of inirungen, prerungen and reprungen*/
#define MAXFIXATIONS	10000000				/*maximum number of fixations permitted in a single initrun or prerun or reprun*/
...
*seq_dat, *temp_seq_dat, *anc_seq_dat, *prev_seq_dat;
...
//	site = (long *) memAlloc(MAXSEQLEN + 1, sizeof(long), "");
    ...
    anc_seq_dat	= (struct seq_dat_str *) memAlloc (MAXSEQNUM + 1, sizeof(struct seq_dat_str), "seq array");
...
// 	}
    ...
    if (userPrefp->fitness_u == 1.000) {
        init_MCU = (userPrefp->u02_a)/((userPrefp->u01_a)+(userPrefp->u02_a));
    }
    else if (userPrefp->fitness_u < 1.000) {
        initNes = userPrefp->prerunSeqNum * (1.000 - (userPrefp->fitness_u));
        u_v = (userPrefp->u02_a)/(userPrefp->u02_b);
...
//	mcu = INITMCU;								/* this should not be initialized to 0.50 for MCU model !!!!!!!!!!!!!!!!!!!!!! */
    ...
    /*150610 matsumoto: set the initial population */
    init_genrand(time(NULL));
    double tm_rand1, tm_rand2;
    curSite=1;
    printf ("initial MCU is %f\n", init_MCU);

    while (curSite <= userPrefp->seqLen) {
    LOOP:
        /*tm_rand1=genrand_real3();
        if (tm_rand1<=(init_MCU/2)) {
            seq_dat[1].site[curSite] = '0';
            anc_seq_dat[1].site[curSite] = '0';
        }
        else if ((tm_rand1>0.25) && (tm_rand1<=0.5)) {
            seq_dat[1].site[curSite] = '1';
            anc_seq_dat[1].site[curSite] = '1';
        }
        else if ((tm_rand1>0.5) && (tm_rand1<=0.75)) {
            seq_dat[1].site[curSite] = '2';
            anc_seq_dat[1].site[curSite] = '2';
        }
        else if ((tm_rand1>0.75) && (tm_rand1<=1.0)) {
            seq_dat[1].site[curSite] = '3';
            anc_seq_dat[1].site[curSite] = '3';
        }
        
        tm_rand1=genrand_real3();
        if (tm_rand1<=0.25) {
            seq_dat[1].site[curSite+1] = '0';
            anc_seq_dat[1].site[curSite+1] = '0';
        }
        else if ((tm_rand1>0.25) && (tm_rand1<=0.5)) {
            seq_dat[1].site[curSite+1] = '1';
            anc_seq_dat[1].site[curSite+1] = '1';
        }
        else if ((tm_rand1>0.5) && (tm_rand1<=0.75)) {
            seq_dat[1].site[curSite+1] = '2';
            anc_seq_dat[1].site[curSite+1] = '2';
        }
        else if ((tm_rand1>0.75) && (tm_rand1<=1.0)) {
            seq_dat[1].site[curSite+1] = '3';
            anc_seq_dat[1].site[curSite+1] = '3';
        }
        */
        /* 150620 matsumoto: make codon usage bias */
        /* 151221 matsumoto: for sequence of synonymous site, all site has base composition bias */
        tm_rand1=genrand_real3();
        tm_rand2=genrand_real3();
        if (tm_rand1<=init_MCU) {
            if (tm_rand2<=0.5) {
                seq_dat[1].site[curSite] = '1';
                anc_seq_dat[1].site[curSite] = '1';
            }
            else if (tm_rand2>0.5) {
                seq_dat[1].site[curSite] = '3';
                anc_seq_dat[1].site[curSite] = '3';
            }
        }
        else if (tm_rand1>init_MCU) {
            if (tm_rand2<=0.5) {
                seq_dat[1].site[curSite] = '0';
                anc_seq_dat[1].site[curSite] = '0';
            }
        ...
        /*
        if ((seq_dat[1].site[curSite] == '0') && (seq_dat[1].site[curSite+1] == '2') && (seq_dat[1].site[curSite+2] == '2')) {
            seq_dat[1].site[curSite+2] = '0';
        }
        if ((seq_dat[1].site[curSite] == '0') && (seq_dat[1].site[curSite+1] == '2') && (seq_dat[1].site[curSite+2] == '3')) {
            seq_dat[1].site[curSite+2] = '1';
        }
        if ((seq_dat[1].site[curSite] == '0') && (seq_dat[1].site[curSite+1] == '3') && (seq_dat[1].site[curSite+2] == '2')) {
            seq_dat[1].site[curSite+2] = '0';
        }
        */
        curSite++;
    ...
    seq_dat[1].baseCount0[1] = seq_dat[1].baseCount0[2] = seq_dat[1].baseCount0[3] = 0;
    seq_dat[1].baseCount1[1] = seq_dat[1].baseCount1[2] = seq_dat[1].baseCount1[3] = 0;
    seq_dat[1].baseCount2[1] = seq_dat[1].baseCount2[2] = seq_dat[1].baseCount2[3] = 0;
    seq_dat[1].baseCount3[1] = seq_dat[1].baseCount3[2] = seq_dat[1].baseCount3[3] = 0;
    for(curSite = 1; curSite <= userPrefp->seqLen; curSite++)
    {
        /* For each site increment baseCounts, fix siteData elements */
        if(seq_dat[1].site[curSite] == '1')
        {
            seq_dat[1].baseCount1[sitePosToCodPos(curSite)]++;
            siteData[curSite].ancSite = '1';
            siteData[curSite].freqs1 = userPrefp->prerunSeqNum;
            siteData[curSite].freqs0 = 0;
            siteData[curSite].freqs2 = 0;
            siteData[curSite].freqs3 = 0;
        }
        else if(seq_dat[1].site[curSite] == '2')
        {
            seq_dat[1].baseCount2[sitePosToCodPos(curSite)]++;
            siteData[curSite].ancSite = '2';
            siteData[curSite].freqs2 = userPrefp->prerunSeqNum;
            siteData[curSite].freqs0 = 0;
            siteData[curSite].freqs1 = 0;
            siteData[curSite].freqs3 = 0;
        }
        else if(seq_dat[1].site[curSite] == '3')
        {
            seq_dat[1].baseCount3[sitePosToCodPos(curSite)]++;
            siteData[curSite].ancSite = '3';
            siteData[curSite].freqs3 = userPrefp->prerunSeqNum;
            siteData[curSite].freqs0 = 0;
            siteData[curSite].freqs1 = 0;
            siteData[curSite].freqs2 = 0;
        }
        else if(seq_dat[1].site[curSite] == '0')
        {
            seq_dat[1].baseCount0[sitePosToCodPos(curSite)]++;
            siteData[curSite].ancSite = '0';
            siteData[curSite].freqs0 = userPrefp->prerunSeqNum;
            siteData[curSite].freqs1 = 0;
            siteData[curSite].freqs2 = 0;
            siteData[curSite].freqs3 = 0;
        }
...
//	printf("preRunGen 12a: %0ld\n", preRunGen);
    ...
    {
        /* this may be an invalid approach */
        //		seq_dat[curSeq] = seq_dat[1];
        /* copy data for seq1 to all other sequences */
        for (curSite=1; curSite <= userPrefp->seqLen; curSite++) {
...
// 		fclose(fpOut_MCU);
	...
	/*sample size*/
	if(userPrefp->sampleNum > MINSEQNUM)
		errorOut(("sampleSize(%lu) cannot be greater than MINPOPSIZE(%lu)", userPrefp->sampleNum, (long) MINSEQNUM));
