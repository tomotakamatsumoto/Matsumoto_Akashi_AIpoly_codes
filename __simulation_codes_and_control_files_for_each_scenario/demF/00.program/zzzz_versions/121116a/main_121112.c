...
long *idum=&idumVal;						
char outFileName[MAXLINECHAR+1], outFileName2[MAXLINECHAR+1], outFileName3[MAXLINECHAR+1];			/*name of output file*/
...
FILE *fpOut, *fpOut2, *fpOut3;								/*output file pointer*/
...
{	
	...
	sprintf(outFileName3, "%s_3", outFileName);
	strcat(outFileName, ".");
	strcat(outFileName2, ".");
	strcat(outFileName3, ".");
	strcat(outFileName, outFileExtn);
	strcat(outFileName2, outFileExtn);
	strcat(outFileName3, outFileExtn);
	checkParameters();
	fclose(fpCtl);
	return 0;
}
...
long initData(void)
{
	...
	long site[MAXSEQLEN+1];		/*temporary array to initialize the sequence*/
	...
	fpOut3=fileOpen(outFileName3, "wb");
...
#endif
		if(!(curGenNum%500))
			printf("\tGeneration %ld of %ld done\n", curGenNum, genNum);
	}
